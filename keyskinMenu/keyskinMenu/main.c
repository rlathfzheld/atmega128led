...
// 예1) 키1번이 눌린경우
		...
		if((PINE&BIT1) == 0)	{scan_key=2;scan_count++;}	// PINE & 0x02 =11101110&00000010=0x02,즉 버튼2번 안눌림		
		if((PINE&BIT2) == 0)	{scan_key=3;scan_count++;}	// PINE & 0x04 =11101110&00000100=0x04,즉 버튼3번 안눌림		
		if((PINE&BIT3) == 0)	{scan_key=13;scan_count++;}	// PINE & 0x08 =11101110&00001000=0x08,즉 버튼4번 안눌림		
...
*/
		...
		Delay(10);				
		if((PINE&BIT0) == 0)	{scan_key=4;scan_count++;} // PINE & 0x01 =11011110&00000001=0x00,즉 버튼5번이 눌림
		if((PINE&BIT1) == 0)	{scan_key=5;scan_count++;} // PINE & 0x02 =11011101&00000010=0x00,즉 버튼6번이 눌림			
		if((PINE&BIT2) == 0)	{scan_key=6;scan_count++;} // PINE & 0x04 =11011011&00000001=0x00,즉 버튼7번이 눌림			
		if((PINE&BIT3) == 0)	{scan_key=14;scan_count++;}// PINE & 0x08 =11010111&00000001=0x00,즉 버튼8번이 눌림			

		PORTE = 0xBF;	//scan_out 1011, 세번째 k9,k10,k11,k12를 체크하기 위한 루틴
		Delay(10);				
		if((PINE&BIT0) == 0)	{scan_key=7;scan_count++;}
		if((PINE&BIT1) == 0)	{scan_key=8;scan_count++;}			
		if((PINE&BIT2) == 0)	{scan_key=9;scan_count++;}			
		if((PINE&BIT3) == 0)	{scan_key=15;scan_count++;}			

		PORTE = 0x7F;	//scan_out 0111 ,네번째 k13,k14,k15,k16을 체크하기 위한 루틴
		Delay(10);				
		if((PINE&BIT0) == 0)	{scan_key=11;scan_count++;}
		if((PINE&BIT1) == 0)	{scan_key=10;scan_count++;}			
		if((PINE&BIT2) == 0)	{scan_key=12;scan_count++;}			
...
{
	lcd_display_clear();
	lcd_display_position(2,1);	//스캔된 값을 lcd위라인에 표시,표시하기 위해서는 sprintf()함수를 사용해야함
	lcd_string("*");			//십진수표시된 값을 지우기위한 문자열로 공백뒤에 다시 포지션을 지정해줘야 함
}
...
{	lcd_display_clear();
	lcd_display_position(2,1);	//스캔된 값을 lcd위라인에 표시,표시하기 위해서는 sprintf()함수를 사용해야함
	lcd_string("call plane");
}

void Key44_D(void)
{	lcd_display_clear();
	lcd_display_position(2,1);	//스캔된 값을 lcd위라인에 표시,표시하기 위해서는 sprintf()함수를 사용해야함
	lcd_string("call a car");
}
...
*/
/*
1. atoi : 문자열을 정수형 바꾸어 주는 함수

    원형 : int atoi(const char *string);
...
UpdateData(TRUE);
usData = atoi(m_strE4); // m_strE4 Edirt 창에서 입력한 문자열 수자를 숫자로 바꾸기

 

2. itoa : 정수형을 문자열로 꾸는 함수

    원형: char *itoa(int value, char *string, int radix);

       - value : 정수값

       - string : 변환된  val 값을 받을 문자열

       - redix : 수자의 진수   10: 10진수, 16:16진수
